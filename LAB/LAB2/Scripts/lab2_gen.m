%erotima d1
s=tf('s')
Gs=0.87/(0.135*s^2+0.74*s+1) %sinartisi metaforas 
Ds=pidstd(4.5, 1.344)       %PI controller CHR Method
figure(1)
step(Gs)
...
x = square(0.43*(t-1));     %gia thn anaparastash
s = tf('s');                %tou systhmatos

systhma = feedback(Ds*Gs, 1);
u=2*x+3;               %tetargwnikos polwmenos palmos
lsim(systhma,u,t);
axis([0 60 0 10])
title('Vimatikh apokrish gennhtrias me PI Controller')
...
Ds_Tsum=pidstd(0.57,0.37);
systhma_Tsum=feedback(Ds_Tsum*Gs,1);
figure(7);
lsim(systhma_Tsum,u,t);
title('Vimatikh apokrish gennhtrias me PI Controller//Tsum')
axis([0 60 0 10]);
